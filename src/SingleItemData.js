...
// end
  ...
  return (
    ...
    <div className='grid justify-center items-center w-full lg:w-full  pt-12'>
      <div ><button  onClick={()=>navlink('/')}>GO BACK</button></div>
      <div className=" py-5 sm:px-6 grid md:grid-cols-2 grid-cols-1 mb-5 shadow-2xl   bg-[#f8f8f8] rounded-2xl mt-4 h-42 md:h-28">
        ...
        <h3 className="text-lg font-medium leading-6 text-gray-900 mr-4">status</h3>
        {data.status==="pending"?
        <button className='text-[#ff8f00] font-bold capitalize bg-orange-100 w-24 h-12 rounded-md'>{data.status}</button>
        :<button className='text-lime-500 font-bold capitalize bg-lime-100 w-24 h-12 rounded-md'>{data.status}</button>}
        </div>
        
        <div className='flex justify-center flex-wrap mt-3'>
          <Update data={data} />

        <button className='rounded-3xl bg-[#ec5757] text-[#ffff] w-24 h-12 font-bold ml-2' onClick={()=>removeData(data.hash)}>Delete</button>
        {data.status==="pending"?
        <button className='rounded-3xl bg-[#7c5dfa] text-[#ffff] w-36 h-12 p-2 ml-2 font-[Spartan]  font-medium' onClick={changeStatus}>Mark as Pending</button>
        :null}
        </div>
      </div>
    <div className="overflow-hidden bg-white shadow-2xl sm:rounded-lg rounded-3xl ">
      ...
      <div >
        <h1 className='text-black font-extrabold font-sans'>#{data.hash}</h1>
        <h1 className='text-[#7e88c3] font-medum'>{data.description}</h1>
      </div>
   <div></div>
      <div className='flex flex-col justify-end flex-wrap'>
        <h1 className='text-[#7e88c3] font-medum'>{data.StreetAdrees_from}</h1>
        <h1 className='text-[#7e88c3] font-medum'>{data.city_from}</h1>
        <h1 className='text-[#7e88c3] font-medum'>{data.postCode_from}</h1>
      ...
      <div className='grid grid-cols-2 md:grid-cols-3 p-7 gap-11'>
        <div>
          ...
          <h1 className='text-black font-extrabold font-sans'>{data. dateInvoice}</h1>
          <h1 className='text-[#7e88c3] font-medum'>Payment Due</h1>
          <h1 className='text-black font-extrabold font-sans'>{data. paymentTerm}</h1>
        </div>
        <div>
          <h1 className='text-[#7e88c3] font-medum'>Bill To</h1>
          <h1 className='text-black font-extrabold font-sans'>{data.cliendName_to}</h1>
          ...
          <h1 className='text-[#7e88c3] font-medum'>{data.cliend_country}</h1>
        </div>
        <div>
          <h1 className='text-[#7e88c3] font-medum'>Sent To</h1>
          <h1 className='text-black font-extrabold font-sans'>{data.cliendEmail_to}</h1>
        </div>
      </div>
